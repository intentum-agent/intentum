...
17. Oh My Pi 风格的 transcript：Pi 内建 `read/bash/edit/write/grep/find/ls` 与四个 `intentum_*` Designer tools 都渲染为一个圆角、按状态着色的 frame（pending/running accent、success dim、error red、aborted/timeout warning），header 含状态字形/共享 braille spinner、工具标题、对象与 dim meta；调用流式阶段用参数填充 body（命令、写入内容、对磁盘预览的 diff），结果落地后追加 `Output` 段，区块不跳动；折叠预览保留尾部并给出 `… N earlier lines [ctrl+o: Expand]`（按键跟随 Pi keybinding）。所有 live spinner 共用一个 80ms ticker。内建工具 override 只替换 `renderCall/renderResult`（`execute`、schema、prompt snippet 仍是 Pi 的），因需 session cwd 而在 `session_start` 注册。Designer 推理时 working row 变为 thinking pulse：定宽星芒 `✻ ✼ ❉ ❊ ✺ ✹ ✸ ✶` 以 70–230ms 余弦缓动呼吸，旁边显示 `Thinking · 1.2K · 45.3 toks/s`（速率徽标随 tok/s 从 dim 渐变到 accent；仅在本 block 观测到 token 增量时显示）；text/tool call 开始即恢复 `Designer working`。隐藏的 thinking block 显示 `✻ Thinking`；可见 thinking 折叠为 prose（fenced code → `…`，空 `<!-- -->` 噪声删除）。`INTENTUM_SYMBOLS=ascii` 与 `INTENTUM_REDUCED_MOTION=1` 分别降级字形与动效。见 `src/tui/tool-frame.ts`、`src/tui/transcript-style.ts`、`src/tui/live-ticker.ts`、`src/tui/thinking-pulse.ts`、`src/tui/thinking-display.ts`、`src/tui/thinking-presentation.ts`、`src/tools/transcript/`。
